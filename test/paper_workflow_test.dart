...
void main() {
  ...
  PaperComposer engine() => PaperComposer(
      mcqBank: allMCQs, saqBank: allSAQs, cqBank: allCQs, random: Random(7));
  test(
    ...
    for (final sid in [
      ...
      expect(paper.mcqs.map((q) => q.questionText).toSet().length,
          paper.mcqs.length);
    }
  });
  test('chapter selection and shortages are enforced, not silently shortened',
    ...
    final p = engine().compose(PaperDraft(
    ...
    expect(
        ...
        throwsStateError);
    expect(
        () => engine().compose(const PaperDraft(
    ...
    expect(
        () => engine().compose(const PaperDraft(
            format: PaperFormat.custom, mcqCount: 0, saqCount: 0, cqCount: 0)),
        throwsStateError);
    expect(() => engine().compose(const PaperDraft(subjectId: 'accounting')),
        throwsStateError);
  });
  ...
  test('controller undo redo and autosave retain the actual question selection',
      () async {
  ...
  test('corrupt draft does not crash or overwrite recovery data on dispose',
      () async {
    ...
    expect(
        (await SharedPreferences.getInstance())
            .getString(PaperController.draftKey),
        'not json');
  });
