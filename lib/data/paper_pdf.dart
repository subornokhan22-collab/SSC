...
class PaperPdf {
  ...
  static Future<List<Uint8List>> _renderPages({
    ...
    // ══════════════ OMR SHEET v4 FIXED – NO OVERLAP ══════════════
    if (mcqs.isNotEmpty) {
      begin();
      final pink = const Color(0xFFE91E63);
      final pinkLight = const Color(0xFFFCE4EC);
      final black = const Color(0xFF000000);
      final borderPink = Paint()..color=pink..style=PaintingStyle.stroke..strokeWidth=1.1*_k;

      // corner marks
      canvas.drawRect(Rect.fromLTWH(_margin - 18*_k, _margin - 18*_k, 11*_k, 11*_k), Paint()..color=black);
      canvas.drawRect(Rect.fromLTWH(sw - _margin + 7*_k, _margin - 18*_k, 11*_k, 11*_k), Paint()..color=black);
      canvas.drawRect(Rect.fromLTWH(_margin - 18*_k, sh - _margin + 7*_k, 11*_k, 11*_k), Paint()..color=black);
      canvas.drawRect(Rect.fromLTWH(sw - _margin + 7*_k, sh - _margin + 7*_k, 11*_k, 11*_k), Paint()..color=black);

      y = _margin;
      await para('মাধ্যমিক ও উচ্চমাধ্যমিক শিক্ষাবোর্ড', 13.5, isBold: true, align: TextAlign.center);
      await para('এস.এস.সি পরীক্ষা- ২০২৭', 11, isBold: true, align: TextAlign.center, gapBefore: 1);
      await para('নৈর্ব্যক্তিক অভীক্ষার উত্তরপত্র', 12, isBold: true, align: TextAlign.center, gapBefore: 1);
      y += 2*_k;
      await para('উত্তরপত্রে নির্ধারিত স্থান ব্যতীত কোনো অবাঞ্ছিত দাগ দেয়া বা কিছু লেখা যাবে না', 8.5, isBold: true, align: TextAlign.center, gapBefore: 2);
      await para('অবশ্যই কালো কালির বল-পয়েন্ট কলম দিয়ে বৃত্ত ভরাট করতে হবে', 9, isBold: true, align: TextAlign.center, gapBefore: 1);
      await rule(gapBefore: 4, gapAfter: 6);

      final leftW = contentW * 0.48;
      final rightW = contentW * 0.48;
      final midGap = contentW * 0.04;
      final leftX = _margin;
      final rightX = _margin + leftW + midGap;

      void drawBubble(double cx, double cy, String lab, {bool filled=false}) {
        final r = 5.2*_k;
      ...
      double qBoxH = (leftCount+1)*rowH + 3*_k;
      canvas.drawRect(Rect.fromLTWH(leftX, startY, leftW, qBoxH), borderPink);
      var tpQH = makePainter('প্রশ্ন নং | ক  খ  গ  ঘ', 8.5, isBold: true);
      tpQH.layout(maxWidth: leftW);
      tpQH.paint(canvas, Offset(leftX+4*_k, startY+1*_k));
      double qY = startY + 10*_k;
      for(int i=0;i<leftCount;i++){
        double yy = qY + i*rowH;
        if(i%2==0) canvas.drawRect(Rect.fromLTWH(leftX, yy, leftW, rowH), Paint()..color=pinkLight.withOpacity(0.28));
        var tpNo = makePainter(_bn(i+1), 8);
        tpNo.layout();
        tpNo.paint(canvas, Offset(leftX+4*_k, yy+1.5*_k));
        double bx = leftX + 20*_k;
        for(int o=0;o<4;o++) drawBubble(bx + o*18*_k, yy+5.5*_k, _optionLetters[o]);
      }

      double secondQY = startY + qBoxH + 8*_k;
      if(rightCount>0){
        double qBoxH2 = (rightCount+1)*rowH + 3*_k;
        canvas.drawRect(Rect.fromLTWH(leftX, secondQY, leftW, qBoxH2), borderPink);
        var tpQH2 = makePainter('প্রশ্ন নং (২৬-${_bn(totalQ)}) | ক  খ  গ  ঘ', 8.5, isBold: true);
        tpQH2.layout(maxWidth: leftW);
        tpQH2.paint(canvas, Offset(leftX+4*_k, secondQY+1*_k));
        double qY2 = secondQY + 10*_k;
        for(int i=0;i<rightCount;i++){
          double yy = qY2 + i*rowH;
          int qNo = 26+i;
          if(i%2==0) canvas.drawRect(Rect.fromLTWH(leftX, yy, leftW, rowH), Paint()..color=pinkLight.withOpacity(0.28));
          var tpNo = makePainter(_bn(qNo), 8);
          tpNo.layout();
          tpNo.paint(canvas, Offset(leftX+4*_k, yy+1.5*_k));
          double bx = leftX + 20*_k;
          for(int o=0;o<4;o++) drawBubble(bx + o*18*_k, yy+5.5*_k, _optionLetters[o]);
      ...
      double rx = rightX;
      double ry = startY;
      double rollW = rightW * 0.62;
      double setW = rightW * 0.34;
      double gapR = rightW * 0.04;

      var tpRoll = makePainter('রোল নম্বর', 9, isBold: true, align: TextAlign.center);
      tpRoll.layout(maxWidth: rollW);
      double rollBoxH = 10*11.5*_k + 14*_k;
      canvas.drawRect(Rect.fromLTWH(rx, ry, rollW, rollBoxH), borderPink);
      tpRoll.paint(canvas, Offset(rx + (rollW-tpRoll.width)/2, ry+1.5*_k));
      double ryy = ry + 11*_k;
      for(int col=0;col<6;col++){
        double cx = rx + 7*_k + col*(rollW-10*_k)/6;
        for(int d=0;d<10;d++){
          double cy = ryy + d*11.5*_k + 5*_k;
          drawBubble(cx, cy, '\$d');
        }
      }

      var tpSet = makePainter('সেট কোড', 9, isBold: true, align: TextAlign.center);
      tpSet.layout(maxWidth: setW);
      double setX = rx + rollW + gapR;
      double setBoxH = 4*13*_k + 14*_k;
      canvas.drawRect(Rect.fromLTWH(setX, ry, setW, setBoxH), borderPink);
      tpSet.paint(canvas, Offset(setX + (setW-tpSet.width)/2, ry+1.5*_k));
      double syy = ry + 11*_k;
      bool hasSet = setCode!=null && setCode!.isNotEmpty;
      for(int i=0;i<4;i++){
        double cy = syy + i*13*_k + 5*_k;
        bool filled = hasSet && _optionLetters[i]==setCode;
        drawBubble(setX + setW/2, cy, _optionLetters[i], filled: filled);
      }

      ry = ry + rollBoxH + 10*_k;
      var tpReg = makePainter('রেজিস্ট্রেশন নম্বর', 9, isBold: true, align: TextAlign.center);
      tpReg.layout(maxWidth: rightW);
      double regBoxH = 10*11.5*_k + 14*_k;
      canvas.drawRect(Rect.fromLTWH(rx, ry, rightW, regBoxH), borderPink);
      tpReg.paint(canvas, Offset(rx + (rightW-tpReg.width)/2, ry+1.5*_k));
      double rgy = ry + 11*_k;
      for(int col=0;col<10;col++){
        double cx = rx + 7*_k + col*(rightW-10*_k)/10;
        for(int d=0;d<10;d++){
          double cy = rgy + d*11.5*_k + 5*_k;
          drawBubble(cx, cy, '\$d');
        }
      }
      ...
      tpSub.layout();
      double subW = rightW * 0.48;
      double subH = 10*11.5*_k + 14*_k;
      canvas.drawRect(Rect.fromLTWH(rx, ry, subW, subH), borderPink);
      tpSub.paint(canvas, Offset(rx + (subW-tpSub.width)/2, ry+1.5*_k));
      String subjDigits = '';
      if (subjectCode!=null){
        const bn = ['০','১','২','৩','৪','৫','৬','৭','৮','৯'];
        String s = subjectCode!;
        for(int i=0;i<10;i++) s = s.replaceAll(bn[i], '\$i');
        subjDigits = s.replaceAll(RegExp(r'[^0-9]'), '');
      }
      double sgy = ry + 11*_k;
      for(int col=0;col<3;col++){
        double cx = rx + 7*_k + col*(subW-10*_k)/3;
        String target = col < subjDigits.length ? subjDigits[col] : '';
        for(int d=0;d<10;d++){
          double cy = sgy + d*11.5*_k + 5*_k;
          bool filled = target.isNotEmpty && target=='\$d';
          drawBubble(cx, cy, '\$d', filled: filled);
        }
      }

      double sigW = rightW * 0.48;
      double sigX = rx + subW + 8*_k;
      double sigH = subH;
      canvas.drawRect(Rect.fromLTWH(sigX, ry, sigW, sigH), borderPink);
      var tpSig = makePainter('কক্ষ পরিদর্শকের স্বাক্ষর', 8, isBold: true, align: TextAlign.center);
      tpSig.layout(maxWidth: sigW-4*_k);
      tpSig.paint(canvas, Offset(sigX + (sigW-tpSig.width)/2, ry+4*_k));
      var tpSig2 = makePainter('ও তারিখ', 8, align: TextAlign.center);
      tpSig2.layout(maxWidth: sigW-4*_k);
      tpSig2.paint(canvas, Offset(sigX + (sigW-tpSig2.width)/2, ry+16*_k));

      double ruleY = ry + subH + 12*_k;
      if(ruleY < sh - _margin - 80*_k){
        var tpRuleHead = makePainter('নিয়মাবলিঃ', 9, isBold: true);
        tpRuleHead.layout();
        tpRuleHead.paint(canvas, Offset(rx, ruleY));
        ruleY += 12*_k;
        final rules = [
          '১। বৃত্ত এমনভাবে ভরাট করো যাতে ভিতরের লেখা দেখা না যায়। সঠিক ● ভুল ✕ ○ ✓',
          '২। অবশ্যই কালো বল-পয়েন্ট কলম দিয়ে ভরাট করতে হবে।',
          '৩। উত্তরপত্রে দাগ বা ভাঁজ করা যাবে না।',
          '৪। পরিচ্ছন্ন উত্তরপত্র মেশিনে মূল্যায়ন অপরিহার্য।',
          '৫। সেট কোড ভুল হলে উত্তরপত্র বাতিল হবে।',
        ];
        for(final r in rules){
          if(ruleY > bottomY - 10*_k) break;
          var tpR = makePainter(r, 7.5);
          tpR.layout(maxWidth: contentW - 10*_k);
          tpR.paint(canvas, Offset(_margin, ruleY));
          ruleY += tpR.height + 2.5*_k;
        }
      }
