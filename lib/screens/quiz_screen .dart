...
class _QuizScreenState extends State<QuizScreen> {
  ...
  void initState() {
    ...
    // Custom (model test / AI generated): 30 questions get 30 minutes.
    // Mixed full quiz (no subject): 15 minutes as before.
    // Subject/chapter quiz: 1 minute per question.
  ...
  List<Question> _buildQuestionSet() {
    ...
    if (widget.subjectId == null) {
      ...
      pool = allMCQs
          .where((q) => q.subjectId == widget.subjectId && q.chapter == widget.chapter)
          .toList();
    } else {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(widget.customTitle ?? 'প্রশ্ন ${_current + 1}/${_questions.length}'),
        actions: [
          Padding(
            ...
            children: [
              if (widget.customTitle != null)
                Padding(
                  ...
                  child: Text(
                    'প্রশ্ন ${_current + 1}/${_questions.length}',
                    style: TextStyle(
                    ...
                    Row(
                      children: [
                        Icon(correct ? Icons.check_circle : Icons.cancel,
                            size: 18, color: correct ? Colors.green : Colors.red),
                        const SizedBox(width: 6),
                        Expanded(
                            child: Text('${i + 1}. ${q.questionText}',
                                style:
                                    const TextStyle(fontWeight: FontWeight.w600))),
                      ],
                    ),
                    const SizedBox(height: 8),
                    Text('সঠিক উত্তর: ${q.options[q.correctIndex]}',
                        style: const TextStyle(color: Colors.green)),
                    if (userAns != null && !correct)
                      Text('আপনার উত্তর: ${q.options[userAns]}',
                          style: const TextStyle(color: Colors.red)),
                    if (userAns == null)
                      const Text('উত্তর দেওয়া হয়নি',
