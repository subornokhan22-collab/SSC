...
class _AiToolsScreenState extends State<AiToolsScreen> {
  ...
  Widget build(BuildContext context) => Scaffold(
      appBar: AppBar(title: const Text('AI Tools'), actions: [
        PopupMenuButton<String>(
            ...
            itemBuilder: (_) => const [
                  PopupMenuItem(
                      value: 'chat', child: Text('Open chat & attachments'))
                ]),
      ]),
      body: ListView(padding: const EdgeInsets.all(20), children: [
        Text('A teaching task, not a chat prompt.',
                  ...
                  Expanded(
                      child: DropdownButtonFormField<int>(
                          ...
                          items: [
                            for (var i = 1; i <= 10; i++)
                              DropdownMenuItem(
                                  value: i, child: Text('$i questions'))
                          ],
        ...
        if (c.summary != null) ...[
          ...
          for (final f in c.findings)
            Card(
                child: ListTile(
                    title: Text(f['title']!), subtitle: Text(f['detail']!)))
        ],
        if (c.questions.isNotEmpty) ...[
          ...
          for (var i = 0; i < c.questions.length; i++)
            Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              if (c.duplicates.any((h) => h.generated.id == c.questions[i].id))
                const Padding(
                    padding: EdgeInsets.all(8),
                    child: Text(
              ...
              onPressed: !c.canUse
                  ...
                  : () {
                      if (widget.forSelection)
                        ...
                        Navigator.pushNamed(context, AppRoutes.createPaper,
                            arguments: CreatePaperArgs(
                                subjectId: subject, questions: c.questions));
                    },
              icon: const Icon(Icons.description_outlined),
              label: Text(widget.forSelection
