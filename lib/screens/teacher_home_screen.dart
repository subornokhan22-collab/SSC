...
class _TeacherHomeScreenState extends State<TeacherHomeScreen>
  ...
  Future<void> create({bool quick = false}) async {
    await Navigator.pushNamed(context, AppRoutes.createPaper,
        arguments: quick
            ? const CreatePaperArgs(
                subjectId: 'physics', format: PaperFormat.board)
            : null);
    if (mounted) load();
  }
      ...
      child: Scaffold(
          ...
          bottomNavigationBar: NavigationBar(
              ...
              destinations: const [
                ...
                NavigationDestination(
                    icon: Icon(Icons.auto_awesome_outlined), label: 'AI Tools'),
              ])));
  Widget home() => Scaffold(
      appBar: AppBar(title: const Text("Tutor’s Desk"), actions: [
        IconButton(
            tooltip: 'Settings',
            onPressed: () async {
      ...
      body: RefreshIndicator(
          ...
          child: ListView(padding: const EdgeInsets.all(20), children: [
            ...
            const Text('Prepare a paper. Review the answers. Start your class.',
                        ...
                        children: [
                          ...
                          SizedBox(
                              ...
                              child: FilledButton.icon(
                                  ...
                                  label: const Text('Create / resume paper'))),
                          TextButton(
                              onPressed: () => create(quick: true),
                              child:
                                  const Text('Start a Physics Model Test →')),
                        ]))),
            const SizedBox(height: 12),
            Row(children: [
              Expanded(
                  child: quickAction(Icons.document_scanner_outlined,
                      'Scan OMR', 'Review & grade', () => select(2))),
              const SizedBox(width: 12),
              Expanded(
                  child: quickAction(Icons.auto_awesome_outlined, 'AI Tools',
                      'Create · Improve · Check', () => select(3)))
            ]),
            const SizedBox(height: 24),
            Row(children: [
              Text('Recent papers',
              ...
              const Spacer(),
              TextButton(
                  onPressed: () => select(1), child: const Text('View all'))
            ]),
            if (loading)
            ...
            for (final entry in recent)
              Card(
                  child: ListTile(
                      ...
                      subtitle: Text(
                          '${entry.subject} · ${entry.pages} pages · ${entry.createdAt.day}/${entry.createdAt.month}'),
