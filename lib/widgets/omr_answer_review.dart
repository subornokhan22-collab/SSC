...
class OmrAnswerReview extends StatelessWidget {
  ...
  const OmrAnswerReview(
      {super.key, required this.result, required this.onChanged});
  @override
  Widget build(BuildContext context) =>
      Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text('Review the detected answers',
  ...
  Widget _row(int i) {
    ...
    final uncertain =
        result.answers[i] < 0 || gap < OMrScanner.minOptionMargin + 0.08;
        ...
        child: Padding(
            ...
            child:
              ...
              Wrap(spacing: 6, children: [
                for (final answer in [0, 1, 2, 3, -1, -2])
                  ChoiceChip(
                      label: Text(answer == -1
                          ? 'Blank'
                          : answer == -2
                      ...
                      onSelected: (_) {
                        final next = List<int>.of(result.answers)..[i] = answer;
                        onChanged(result.corrected(answers: next));
                      })
              ]),
              if (inks.length == 4)
                Row(children: [
...
Future<OmScanResult?> reviewOmrSheet(BuildContext context, OmScanResult initial,
        {String title = 'Review OMR sheet'}) =>
    Navigator.push<OmScanResult>(context,
        ...
        if (result.rectifiedJpeg != null)
          SizedBox(
              ...
              child: InteractiveViewer(
                  maxScale: 5,
                  child: Image.memory(result.rectifiedJpeg!,
                      fit: BoxFit.contain))),
        OmrAnswerReview(
            result: result, onChanged: (r) => setState(() => result = r)),
      ]),
