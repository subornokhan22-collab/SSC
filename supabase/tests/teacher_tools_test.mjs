...
const question=()=>({chapter:'Motion',questionText:'What is the unit of speed?',options:['m/s','m','s','kg'],correctIndex:0,explanation:'Speed is distance divided by time.',difficulty:'easy'});
...
test('two actual model passes precede checked status; checker cannot see generated key',async()=>{
  ...
  const result=await runTeacherTool(request(),async(system,input,schema,validator)=>{
    ...
    return validator?{checks:[{index:0,correctIndex:0,valid:true,reason:'distance divided by time'}]}:{questions:[question()]};
...
test('Check / Explain use structured findings, not an answer-verification badge',async()=>{
  const result=await runTeacherTool({...request(),action:'explain',text:'Explain speed'},async()=>({summary:'Distance per unit time.',findings:[{title:'Formula',detail:'v = d / t'}]}),()=>{});
