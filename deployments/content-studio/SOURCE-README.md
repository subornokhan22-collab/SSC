...
5. Deploy `supabase/functions/admin-content` for optional AI structuring/review. It verifies the caller’s authenticated user and administrator membership. Configure `GEMINI_API_KEY` as an Edge Function secret, not a frontend setting. Manual import works without it. The APK uses a **different** function, `mimi`: deploy `supabase/functions/mimi` as well for AI Tools/chat. Both use the same project-level `GEMINI_API_KEY`. For dashboard-only deployment, complete single-file bundles are available in `deployments/content-studio/`.
6. Upload the static `web-admin` directory, **including `english-upload.js`, `vendor/`, `catalog.json`, and the worker**, to your HTTPS hosting. Exclude `node_modules`, tests and reports. Configure `.mjs` as JavaScript. CSP is in `index.html`; a hosting-level equivalent is recommended. API traffic goes directly to the configured Supabase HTTPS project, never localhost.
...
- **English Papers → Upload English Paper (PDF / Image)** accepts one PDF (up to 10 pages) or up to 10 JPG/PNG/WebP page photos, up to 20 MB total. A page preview and image ordering controls are available before drafting. PDF.js extracts selectable text locally.
- **Extract PDF / images with AI** sends resized page images to the authenticated `admin-content` function only after confirmation. This supports AI-assisted transcription of photos and scanned PDFs; it is not guaranteed OCR. Inspect every passage, table and question. Blurry/unreadable parts require manual repair. The function must be redeployed with the new attachment support; manual import does not need AI. Offline demo never sends images to the server.
- AI requests are limited to 10 JPEG/PNG/WebP pages and 3 MB combined decoded image bytes. SVGs, remote URLs, spoofed MIME signatures and oversized requests are rejected. Source originals remain in the browser session; saved drafts contain structured content and filenames, not binary originals. Generic question figures retain their separate upload workflow. English inline-image embedding is still not a model feature.
